...
// ---------------- Motor Speed ----------------
// 0-255 range. Set to 180 for strong starting torque to turn the belt motor.
int MOTOR_SPEED = 180;
...
void sendReading(float vibration, float rpm, float current, unsigned long particles) {
  ...
  if (code > 0) {
    ...
    // Process live remote motor control commands from Web Dashboard
    if (response.indexOf("\"conveyorStatus\":\"RUNNING\"") != -1 || response.indexOf("\"targetSpeed\":200") != -1) {
      if (MOTOR_SPEED < 150) {
        MOTOR_SPEED = 200;
        setMotorSpeed(200);
      }
    } else if (response.indexOf("\"conveyorStatus\":\"STOPPED\"") != -1 || response.indexOf("\"conveyorStatus\":\"EMERGENCY_STOP\"") != -1) {
      if (MOTOR_SPEED != 0) {
        MOTOR_SPEED = 0;
        setMotorSpeed(0);
      }
